...
<template x-teleport="body">
    <div x-show="showModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <form id="categoryForm" @submit.prevent="submitForm($data)" x-show="showModal"
            ...
            <div
                ...
                <div>
                    ...
                    <h3 class="text-base md:text-lg font-black text-white mt-0.5"
                        x-text="isEdit ? 'Modificar Categoría' : 'Registrar Categoría'">Registrar Categoría</h3>
                </div>
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-grow">
                ...
                <div>
                    <label for="name"
                        class="block text-[10px] font-black text-primary uppercase tracking-widest mb-1.5">Nombre de la
                        Categoría</label>
                    <input type="text" id="name" x-model="categoryName"
                        ...
                        placeholder="Ej: Globos y Detalles">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="seo_title"
                            ...
                            SEO</label>
                        <input type="text" id="seo_title" x-model="categorySeoTitle"
                            class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-xl px-4 py-3 focus:outline-none focus:border-primary transition"
                            placeholder="Ej: Globos y Detalles para Fiestas">
                    ...
                    <div>
                        <label for="seo_description"
                            ...
                            SEO</label>
                        <input type="text" id="seo_description" x-model="categorySeoDescription"
                            class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-xl px-4 py-3 focus:outline-none focus:border-primary transition"
                            placeholder="Ej: Encuentra globos, detalles y más para tu evento.">
                ...
                <div>
                    <label for="icon"
                        class="block text-[10px] font-black text-primary uppercase tracking-widest mb-1.5">Icono
                        (FontAwesome)</label>
                    <div class="flex gap-2">
                        <div class="relative flex-grow">
                            <input type="text" id="icon" x-model="categoryIcon"
                                class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-xl px-4 py-3 focus:outline-none focus:border-primary transition"
                                placeholder="Ej: fa-fire, fa-star, fa-glass-water">
                        </div>
                        <div
                            class="w-12 h-12 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 shrink-0 shadow-sm">
                            <i class="fas text-lg" :class="categoryIcon || 'fa-folder'"
                                :style="'color: ' + categoryColor"></i>
                        </div>
                    </div>
                    <p class="text-[9px] text-slate-400 mt-1 font-semibold">Ingresa una clase de FontAwesome 6 o usa los
                        pre-selectores rápidos a continuación.</p>

                    <!-- Pre-selectores Rápidos -->
                    <div class="mt-2 flex flex-wrap gap-1.5">
                        <button type="button" @click="categoryIcon = 'fa-fire'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                            <i class="fas fa-fire text-amber-500"></i> Clásico
                        </button>
                        <button type="button" @click="categoryIcon = 'fa-star'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                            <i class="fas fa-star text-yellow-500"></i> Estrella
                        </button>
                        <button type="button" @click="categoryIcon = 'fa-glass-water'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                            <i class="fas fa-glass-water text-blue-500"></i> Bebida
                        </button>
                        <button type="button" @click="categoryIcon = 'fa-candy-cane'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                            <i class="fas fa-candy-cane text-pink-500"></i> Dulce
                        </button>
                        <button type="button" @click="categoryIcon = 'fa-basket-shopping'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                            <i class="fas fa-basket-shopping text-indigo-500"></i> Conveniencia
                        </button>
                        <button type="button" @click="categoryIcon = 'fa-hamburger'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                            <i class="fas fa-hamburger text-amber-600"></i> Hamburguesa
                        </button>
                        <button type="button" @click="categoryIcon = 'fa-pizza-slice'"
                            class="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 transition flex items-center gap-1">
                ...
                <div>
                    <label for="color"
                        class="block text-[10px] font-black text-primary uppercase tracking-widest mb-1.5">Color de la
                        Categoría</label>
                    <div class="flex items-center gap-3">
                        <input type="color" id="color" x-model="categoryColor"
                            class="w-12 h-12 cursor-pointer bg-transparent border-0 outline-none rounded-xl shrink-0">
                        <input type="text" x-model="categoryColor"
                            class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-xl px-4 py-3 focus:outline-none focus:border-primary transition"
                            placeholder="#E60067">
                    </div>
                    <p class="text-[9px] text-slate-400 mt-1 font-semibold">Selecciona un color para teñir la tarjeta y
                        sus badges distintivos.</p>
                </div>

                <!-- Estado -->
                <div>
                    <label for="status"
                        class="block text-[10px] font-black text-primary uppercase tracking-widest mb-1.5">Estado</label>
                    <select id="status" x-model="categoryStatus"

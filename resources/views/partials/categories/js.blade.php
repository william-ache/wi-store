            ...
            columns: [
                { 
                    ...
                    render: function(data, type, row) {
                        const iconClass = row.icon ? row.icon : 'fa-folder';
                        const colorVal = row.color ? row.color : 'var(--color-primary)';
                        return `
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-xl flex items-center justify-center text-white shrink-0 shadow-sm" style="background-color: ${colorVal}">

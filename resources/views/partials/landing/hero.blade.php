    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-10 items-center">
        ...
        <div class="lg:col-span-6 text-center lg:text-left space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-xl mx-auto lg:mx-0 text-left">
                <div class="landing-benefit-card flex items-start gap-3 bg-white/[0.04] border border-white/10 rounded-2xl p-3 backdrop-blur-sm">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-2">
                <a href="/register"
                   class="w-full sm:w-auto text-center bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-500 hover:to-cyan-400 hover:scale-[1.02] active:scale-[0.98] text-white font-black px-8 py-4 rounded-2xl shadow-[0_0_25px_rgba(168,85,247,0.35)] text-sm md:text-base">
                    Probar gratis 7 días
                </a>
                <button type="button" @click="scrollTo('explorar')"
                        class="w-full sm:w-auto text-center border border-white/15 bg-white/5 hover:bg-white/10 hover:scale-[1.01] active:scale-[0.98] text-white font-extrabold px-8 py-4 rounded-2xl text-sm md:text-base">
        ...
        <div class="lg:col-span-6 relative flex flex-col items-center"
             @mouseenter="heroDemoPaused = true"
             @mouseleave="heroDemoPaused = false"
             @focusin="heroDemoPaused = true"
             @focusout="heroDemoPaused = false">
            <div class="flex flex-wrap justify-center gap-2 mb-5 w-full max-w-md">
                <button type="button" @click="setHeroDemoStep(1)"
                        :class="heroDemoStep === 1 ? 'landing-step-pill is-active' : ''"
                        class="landing-step-pill text-[11px] font-bold px-3 py-2 rounded-full border border-slate-700 text-slate-400">
                    ① Productos
                </button>
                <button type="button" @click="setHeroDemoStep(2)"
                        :class="heroDemoStep === 2 ? 'landing-step-pill is-active' : ''"
                        class="landing-step-pill text-[11px] font-bold px-3 py-2 rounded-full border border-slate-700 text-slate-400">
                    ② Colores
                </button>
                <button type="button" @click="setHeroDemoStep(3)"
            ...
            <div class="relative w-full max-w-[340px] landing-float">
                <div class="absolute -top-3 -right-2 z-20 bg-cyan-500 text-slate-950 text-[10px] font-black px-3 py-1.5 rounded-full shadow-lg transition-transform duration-500"
                     :class="heroDemoStep === 1 ? 'scale-100' : 'scale-95 opacity-90'">
                    Toca los pasos ↑
                </div>
                <div class="bg-slate-900 border-[10px] border-slate-800 rounded-[2.5rem] p-4 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden transition-shadow duration-500 hover:shadow-[0_0_60px_rgba(168,85,247,0.2)]">
                    <div class="bg-slate-50 rounded-[1.75rem] overflow-hidden min-h-[380px] flex flex-col">
                        <div class="landing-hero-demo-header p-4 text-center text-white transition-all duration-500"
                             :class="{
                                 'bg-gradient-to-r from-purple-600 to-cyan-500': heroDemoStep === 1,
                                 'bg-gradient-to-r from-fuchsia-600 to-amber-500': heroDemoStep === 2,
                                 'bg-gradient-to-r from-emerald-600 to-cyan-500': heroDemoStep === 3
                             }">
                            <div class="w-14 h-14 mx-auto rounded-full bg-white/20 flex items-center justify-center text-lg font-black mb-2 transition-transform duration-500"
                                 :class="heroDemoStep === 2 ? 'scale-110 ring-4 ring-white/30' : 'scale-100'">YB</div>
                            <p class="font-black text-sm">Tu tienda aquí</p>
                        </div>

                        <div class="landing-hero-demo-stage">
                            <div class="landing-hero-demo-panel"
                                 x-show="heroDemoStep === 1"
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    @foreach ([['🍔', 'Combo', '$12'], ['🥤', 'Bebida', '$3'], ['🍰', 'Postre', '$8'], ['⭐', 'Especial', '$15']] as $product)
                                        <div class="bg-white rounded-xl p-3 border border-slate-100 text-center shadow-sm transition-transform duration-300 hover:scale-[1.02]">
                            ...
                            <div class="landing-hero-demo-panel"
                                ...
                                <div class="flex justify-center gap-3 mb-4">
                                    <span class="w-12 h-12 rounded-full bg-[#E60067] ring-4 ring-purple-200 shadow-md transition-transform duration-500 scale-110"></span>
                                    <span class="w-12 h-12 rounded-full bg-[#C6A100] ring-2 ring-slate-200 transition-transform duration-300 hover:scale-105"></span>
                                    <span class="w-12 h-12 rounded-full bg-[#0b0f19] ring-2 ring-slate-200 transition-transform duration-300 hover:scale-105"></span>
                                </div>
                                <p class="text-xs text-slate-500 text-center">Así se verá tu menú con tu estilo</p>
                            </div>

                            <div class="landing-hero-demo-panel justify-center"
                                 x-show="heroDemoStep === 3"
                                 x-transition:enter="landing-motion-enter"
                                 x-transition:enter-start="opacity-0 translate-y-3 scale-[0.98]"
                                 x-transition:enter-end="opacity-100 translate-y-0 scale-100"
                                 x-transition:leave="landing-motion-leave"

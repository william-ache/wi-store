...
<section id="testimonios" class="py-14 md:py-20 mt-10 md:mt-14 relative z-10"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-8 md:mb-10">
            <span class="text-[10px] font-black uppercase tracking-widest text-amber-300/90 bg-amber-500/10 border border-amber-500/25 px-3 py-1 rounded-full">
                Comercios reales
            </span>
            <h2 class="text-2xl md:text-4xl font-black text-white tracking-tight mt-3">Lo que dicen quienes usan WI-Store</h2>
            <p class="text-sm text-slate-300 mt-2 max-w-xl mx-auto">Tiendas que calificaron la plataforma después de vender con su menú digital.</p>
        </div>
        ...
        <div class="flex flex-wrap items-center justify-center gap-2 mb-8">
            <button type="button" @click="setFilter('all')"
                ...
                class="landing-step-pill text-[11px] font-bold px-3 py-1.5 rounded-full border border-slate-700 text-slate-300 transition-all">
                Todas
            </button>
            @foreach ([5, 4, 3, 2, 1] as $stars)
                <button type="button" @click="setFilter('{{ $stars }}')"
                    :class="starFilter === '{{ $stars }}' ? 'landing-step-pill is-active' : ''"
                    class="landing-step-pill text-[11px] font-bold px-3 py-1.5 rounded-full border border-slate-700 text-slate-300 transition-all inline-flex items-center gap-1">
                    {{ $stars }}
        ...
        <div x-show="filtered.length === 0" x-cloak class="text-center py-12 rounded-2xl border border-dashed border-slate-700 bg-slate-900/30">
            <p class="text-slate-400 text-sm">No hay reseñas con esa calificación todavía.</p>
        </div>
        ...
        <div x-show="filtered.length > 0" class="relative flex flex-col items-center">
                    ...
                    x-transition:leave-end="opacity-0 -translate-x-4"
                        ...
                        <div class="pt-3 px-4 pb-4 flex-grow flex flex-col">
                            <div class="flex items-start justify-between gap-2 mb-2">
                                <h3 class="text-base font-black text-white leading-tight line-clamp-1" x-text="item.shop_name"></h3>
                            ...
                            <p class="text-xs font-black text-white leading-snug line-clamp-2 mb-1.5" x-text="item.title"></p>
                            <p class="text-[10px] text-slate-400 leading-relaxed line-clamp-3 mb-2" x-text="item.comment"></p>
                            <p class="text-[9px] text-slate-500 font-semibold" x-text="item.created_at"></p>

                            <template x-if="item.shop_slug">
                                <a :href="'/' + item.shop_slug"
                                    class="mt-4 block w-full text-center bg-slate-800/50 hover:bg-purple-600 text-white font-bold py-2.5 rounded-xl transition-all duration-300 text-[11px] shadow-sm">
                                    Ver tienda
                                </a>
                            </template>
                        </div>
            ...
            <div x-show="filtered.length > 1" class="flex items-center justify-center gap-4 mt-6">
                <button type="button" @click="prev()" class="w-10 h-10 rounded-full border border-white/15 bg-white/5 hover:bg-purple-500/20 text-white transition-all" aria-label="Anterior">
                    <i class="fas fa-chevron-left text-xs"></i>
                </button>
                <span class="text-[11px] font-bold text-slate-400 tabular-nums" x-text="(slide + 1) + ' / ' + filtered.length"></span>
                <button type="button" @click="next()" class="w-10 h-10 rounded-full border border-white/15 bg-white/5 hover:bg-purple-500/20 text-white transition-all" aria-label="Siguiente">

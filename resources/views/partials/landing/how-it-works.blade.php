<section id="como-funciona" class="py-14 md:py-20 relative overflow-hidden z-10 border-t border-white/5">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="mb-6 md:mb-8 text-center">
            <span class="text-[10px] font-black uppercase tracking-widest text-cyan-300 bg-cyan-500/10 border border-cyan-500/25 px-3 py-1 rounded-full">3 pasos + tutorial</span>
            <h2 class="text-2xl md:text-4xl font-black text-white tracking-tight mt-3">Así de fácil funciona</h2>
            <p class="text-xs md:text-sm text-slate-400 mt-2 max-w-xl mx-auto">
        ...
        <div class="flex sm:hidden gap-2 justify-center mb-4 flex-wrap">
            @foreach ([1 => 'Productos', 2 => 'Estilo', 3 => 'Pedidos'] as $num => $label)
                <button type="button" @click="activeHowStep = {{ $num }}"
                        ...
                        class="landing-step-pill text-[10px] font-bold px-3 py-1.5 rounded-full border border-slate-700 text-slate-400">
                    {{ $num }}. {{ $label }}
                </button>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 md:mb-8 max-w-5xl mx-auto">
            <button type="button" @click="activeHowStep = 1"
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        <i class="fas fa-box-open text-purple-400 text-sm"></i>
                        <h3 class="text-sm font-black text-white">Sube tus productos</h3>
                    </div>
                    <p class="text-[11px] text-slate-400 leading-snug">Foto, nombre, precio y categorías en tu mostrador digital.</p>
                </div>
            </button>
            ...
            <button type="button" @click="activeHowStep = 2"
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        <i class="fas fa-palette text-cyan-400 text-sm"></i>
                        <h3 class="text-sm font-black text-white">Ponle tu estilo</h3>
                    </div>
                    <p class="text-[11px] text-slate-400 leading-snug">Logo, portada y colores de tu marca al instante.</p>
                </div>
            </button>
            ...
            <button type="button" @click="activeHowStep = 3"
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <h3 class="text-sm font-black text-white">Recibe pedidos</h3>
                    </div>
                    <p class="text-[11px] text-slate-400 leading-snug">Comparte tu enlace; el pedido llega ordenado a WhatsApp o Telegram.</p>
                </div>
            </button>
        </div>

        {{-- Video ancho completo debajo --}}
